...
#Example: python3 insurance_ec2_start_stop.py start  --> this will start  35.163.68.70 instance 
...
project_name = 'prod-telit-client-access'
usage = "python3.4 insurance_ec2_start_stop.py start|stop|status"
instance_id = 'i-027caa8bb801f7cf7'
#os.system("aws ec2 describe-instances --instance-id i-0944e284da9653602 --query 'Reservations[*].Instances[*].PublicIpAddress' > out.txt")
#reader = open("out.txt")
region = "us-west-2" 
start_msg = 'starting' + ' ' + project_name + ' ' + 'instance'
stop_msg = 'stopping' + ' ' + project_name + ' ' +'instance'
user = os.environ['SUDO_USER']
TO = ['vishnu.pb@orzota.com','devops@orzota.com','aberami.g@orzota.com','anitha.m@orzota.com']
def send_mail(TEXT,TEXT1):
    SUBJECT = project_name +'Instance Start/Stop Alert'
    #TEXT = serv + ' ' + "IOT 35.163.68.70 instance has been started by", user
    TEXT = TEXT + TEXT1
    #print (TEXT)
    #print (' '.join(TEXT))
    # Gmail Sign In
...
if len(sys.argv) < 2:
    ...
    if ins_status(instance_id) == "running" or ins_status(instance_id) == "pending":
        print (project_name +' ' +"35.163.68.70 instance is already in running state please check mail about who has started")
    else:
        print (start_msg)
        start_ins(instance_id)
        send_mail(project_name +' ' +"35.163.68.70 instance has been started by" + ' ', user)
elif sys.argv[1] == "stop":
    if ins_status(instance_id) == "shutting-down" or ins_status(instance_id) == "stopped" or ins_status(instance_id) == "stopping":
        print (project_name +' ' +"35.163.68.70 instance is already in stopped state. Please check mail about who has started")
    else:
        print (stop_msg)
        stop_ins(instance_id)
        send_mail(project_name +' ' +"35.163.68.70 instance has been stopped by" + ' ', user)
elif sys.argv[1] == "status":
    if ins_status(instance_id) == "running" or ins_status(instance_id) == "pending":
        print (project_name +' ' +"35.163.68.70 instance is running")
    elif ins_status(instance_id) == "shutting-down" or ins_status(instance_id) == "stopped" or ins_status(instance_id) == "stopping":
        print (project_name +' ' +"35.163.68.70 instance is in stopped state")
else:
    print (usage)
