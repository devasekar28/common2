...
project_name = 'Cigna Development 34.211.168.92'
usage = "python3.4 insurance_ec2_start_stop.py start|stop|status"
instance_id = 'i-0be64f190a1d08a62'
#os.system("aws ec2 describe-instances --instance-id i-0be64f190a1d08a62 --query 'Reservations[*].Instances[*].PublicIpAddress' > out.txt")
#reader = open("out.txt")
region = "us-west-2" 
...
def send_mail(TEXT,TEXT1):
    ...
    #TEXT = serv + ' ' + "IOT instancehas been started by", user
    TEXT = TEXT + TEXT1
    #print (TEXT)
    #print (' '.join(TEXT))
    # Gmail Sign In
...
if len(sys.argv) < 2:
    ...
    if ins_status(instance_id) == "running" or ins_status(instance_id) == "pending":
        print (project_name + ' ' +"instanceis already in running state please check mail about who has started")
    else:
        print (start_msg)
        start_ins(instance_id)
        send_mail(project_name + ' ' + "instancehas been started by" + ' ', user)
elif sys.argv[1] == "stop":
    if ins_status(instance_id) == "shutting-down" or ins_status(instance_id) == "stopped" or ins_status(instance_id) == "stopping":
        print (project_name + ' ' + "instanceis already in stopped state. Please check mail about who has started")
    else:
        print (stop_msg)
        stop_ins(instance_id)
        send_mail(project_name + ' ' + "instancehas been stopped by" + ' ', user)
elif sys.argv[1] == "status":
    if ins_status(instance_id) == "running" or ins_status(instance_id) == "pending":
        print (project_name + ' ' + "instanceis running")
    elif ins_status(instance_id) == "shutting-down" or ins_status(instance_id) == "stopped" or ins_status(instance_id) == "stopping":
        print (project_name + ' ' + "instanceis in stopped state")
else:
    print (usage)
